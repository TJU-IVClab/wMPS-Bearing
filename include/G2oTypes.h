...
public:
    ...
    virtual void oplusImpl(const double* update_, int iteration = 0){
...
class EdgewMPSTransmitterRegulation : public g2o::BaseUnaryEdge<3,Eigen::Vector3d,g2o::VertexSE3Expmap>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    EdgewMPSTransmitterRegulation(){};
    ...
    virtual void computeError() override {
        ...
        g2o::SE3Quat Twa = v1->estimate();
        _error =  Twa.translation();
    }
};

//wMPS transmitter pose estimation
class EdgewMPSTransmitterSE3Graph : public g2o::BaseUnaryEdge<2,Eigen::Vector2d,g2o::VertexSE3wMPStoCam>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;

    EdgewMPSTransmitterSE3Graph(){};
    ...
    virtual void computeError() override {
        ...
        const g2o::VertexSE3wMPStoCam * v1 = static_cast<const g2o::VertexSE3wMPStoCam*>(_vertices[0]);  //vRT

        Eigen::Matrix3d Rwc = gTwc.rotation().toRotationMatrix();
        Eigen::Vector3d twc = gTwc.translation();
        Eigen::Matrix4d Twc = Eigen::Matrix4d::Identity();
        Twc.block<3,3>(0,0) = Rwc;
        Twc.block<3,1>(0,3) = twc;
        Eigen::Vector4d Pcr = Eigen::Vector4d( mwMPSReceiver[channel].x(), mwMPSReceiver[channel].y(),mwMPSReceiver[channel].z() , 1.0);
        Eigen::Vector4d Pwr4d = Twc * Pcr;
        Eigen::Vector3d Pwr = Pwr4d.block<3,1>(0,0);

        g2o::SE3Quat Twp = v1->estimate();
        Eigen::Matrix3d Rwp = Twp.rotation().toRotationMatrix();
        Eigen::Vector3d Pwp = Twp.translation();
        Eigen::Vector2d obs = BearingAngle;

        Eigen::Vector3d bearing = Pwr - Pwp;
        double norm = sqrt(bearing.x()*bearing.x() + bearing.y()*bearing.y() +bearing.z()*bearing.z());
        bearing = Rwp.transpose() * bearing;
        double norm2 = sqrt(bearing.x()*bearing.x() + bearing.y()*bearing.y() +bearing.z()*bearing.z());
        bearing /= norm2;
        double horizon = 0, vertical = 0;
        Bearing2Angle(bearing, horizon, vertical);

        _error = Eigen::Vector2d(horizon, vertical) - obs;

    }

    void Bearing2Angle(Eigen::Vector3d line, double& horizon, double& vertical)
    {
    ...
    g2o::SE3Quat gTwc;
    int channel;
    map<int, Eigen::Vector3d> mwMPSReceiver;
    Eigen::Vector2d BearingAngle;
};

class EdgewMPSBearing:public g2o::BaseMultiEdge<2,Eigen::Vector2d>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    ...
    virtual void computeError() override
    {
        ...
        Twc.block<3,1>(0,3) = twc;
        Eigen::Vector4d ReceiverPosInCam4d = Eigen::Vector4d( mwMPSReceiver[channel].x(), mwMPSReceiver[channel].y(),mwMPSReceiver[channel].z() , 1.0);
        Eigen::Vector4d ReceiverPosition4d = Twc * ReceiverPosInCam4d;
        Eigen::Vector3d ReceiverPosition3d = ReceiverPosition4d.block<3,1>(0,0);
        Eigen::Matrix3d Rwa = Twa.rotation().toRotationMatrix();
        Eigen::Vector3d twa = Twa.translation();

        Eigen::Vector2d obs = BearingAngle;
        Eigen::Vector3d bearing = ReceiverPosition3d - twa;
        bearing = Rwa.transpose() * bearing;
        bearing.normalize();
        double horizon = 0, vertical = 0;
        Bearing2Angle(bearing, horizon, vertical);
        _error = Eigen::Vector2d(horizon, vertical) - obs;
