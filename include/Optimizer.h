...
public:
    ...
    /*****     wMPSBearing   ******/
    void static GlobalViBundleAdjustement(const std::vector<KeyFrame*> &vpKF, const std::vector<MapPoint*> &vpMP,
                                         vector<cv::Mat>& mvTwa, bool mbMonocular, int nIterations = 5, long weight = 1e5, bool *pbStopFlag=NULL, const unsigned long nLoopKF=0,
                                         const bool bRobust = true);
    bool static GlobalVisualwMPSBundleAdjustment(Map* pMap, vector<cv::Mat>& mvTwa, bool mbMonocular, int nIterations=5, long weight = 1e5, bool *pbStopFlag=NULL,
                                                 const unsigned long nLoopKF=0, const bool bRobust = true);
    int static wMPSBearingPoseOptimization(Frame* pFrame,vector<cv::Mat>& vTwa,long weight);

    //Twa denotes the wMPS pose in camera frame
    void static LocalwMPSDirBA(KeyFrame* pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, vector<cv::Mat>& vTwa, bool bScaleFixFlag, bool bMonocular, list<KeyFrame*> lKF, long weight);
    bool static wMPSInitialization(Map *pMap,Eigen::Matrix4d& T, int Channel, int k);

    template<class T>
    void static addwMPSBearingEdge(g2o::SparseOptimizer&,T,vector<cv::Mat>&,vector<EdgewMPSBearing*>&,long weight);
    void static addwMPSBearingPoseOptimizationEdge(g2o::SparseOptimizer&,Frame *,vector<cv::Mat>&,vector<EdgewMPSBearing*>&,long weight);
