...
bool Optimizer::wMPSInitialization(Map *pMap,Eigen::Matrix4d& Twp, int TrmSpeed, int k)
{
    cout << "wMPSInitialization" <<endl;
    ...
    //    if (pKFi->mnId > maxKFid || pKFi->mwMPSBearing.x() == 0.0)
    //        continue;
    //    //if(num%5 != 0)
    //    //    continue;
    //    g2o::VertexSE3Expmap * vP = new g2o::VertexSE3Expmap();
    //    vP->setEstimate(Converter::toSE3Quat(pKFi->GetPose())); //Tcw 注意在优化的时候改一下
    //    vP->setId(pKFi->mnId);
    //    vP->setFixed(true);
    ...
    Eigen::Matrix3d Rwp = Twp.block<3,3>(0,0);
    Eigen::Vector3d twp = Twp.block<3,1>(0,3);

    auto * vTwp = new g2o::VertexSE3wMPStoCam(k);
    vTwp->setId(0);
    vTwp->setFixed(false);
    vTwp->setEstimate(g2o::SE3Quat(Rwp,twp));
    optimizer.addVertex(vTwp);

    EdgewMPSTransmitterRegulation* er = new EdgewMPSTransmitterRegulation();
    er->setVertex(0,dynamic_cast<g2o::OptimizableGraph::Vertex*>(vTwp));
    Eigen::Matrix<double, 3, 3> erMatrix = Eigen::Matrix<double, 3, 3>::Identity();
    erMatrix = 0.001 * erMatrix;
    er->setInformation(erMatrix);
    optimizer.addEdge(er);

    vector<EdgewMPSTransmitterSE3Graph*> vpei;
    int num = 0;
    int num2= 0;
    for(size_t i=0;i<vpKFs.size();i++)
    {
        num ++;
        KeyFrame* pKFi = vpKFs[i];
        if(pKFi->mwMPSBearing.empty())
            continue;

        for(auto k = 0; k < pKFi->mwMPSBearing.size(); k++)
        {
            int channel = pKFi->mwMPSChannel[k];
            int Trm = pKFi->mwMPSTransmitter[k];
            Eigen::Vector2d BearingAngle = pKFi->mwMPSAngle[k];
            if(Trm != TrmSpeed)
                continue;

            int i = 0;
            identifyTransmitter(Trm,i);

            if (pKFi->mnId > maxKFid || pKFi->mwMPSAngle[k].norm() < 0.001)
                continue;

            g2o::HyperGraph::Vertex* vTwp = optimizer.vertex(0);

            if( !vTwp )
            {
                cout << "Error" << ", "<< vTwp <<endl;
                continue;
            }

            if(pKFi->GetPoseInverse().translation().norm() < 0.01)
                continue;

            EdgewMPSTransmitterSE3Graph* ei = new EdgewMPSTransmitterSE3Graph();
            ei->setVertex(0,dynamic_cast<g2o::OptimizableGraph::Vertex*>(vTwp));
            ei->BearingAngle = BearingAngle;
            ei->channel = channel;
            ei->mwMPSReceiver = pKFi->mwMPSReceiver;
            ei->gTwc = g2o::SE3Quat(Converter::toSE3Quat(pKFi->GetPoseInverse()));
            Eigen::Matrix<double, 2, 2> inforMatrix = Eigen::Matrix<double, 2, 2>::Identity();
            inforMatrix = 1000 * inforMatrix;
            ei->setInformation(inforMatrix);
            optimizer.addEdge(ei);
            vpei.push_back(ei);
            num2 ++;   //Need enough number of Bearing for estimation
        }
    ...
    {
        vector<float> vchi2;
        for (size_t i = 0, iend = vpei.size(); i < iend; i++) {
            EdgewMPSTransmitterSE3Graph *e = vpei[i];
    ...
    if(optimizer.activeEdges().size()<10)
    {
        cout << "optimizer.activeEdges().size() = "<< optimizer.activeEdges().size() << endl;
        Twp.setIdentity();
        return false;
    }

    g2o::SE3Quat RT = vTwp->estimate();      //Twp
    Twp.block<3,3>(0,0) = RT.rotation().toRotationMatrix();
    Twp.block<3,1>(0,3) = RT.translation();

    return true;
}

void Optimizer::addwMPSBearingPoseOptimizationEdge(g2o::SparseOptimizer& optimizer,Frame* pKFi,vector<cv::Mat>& vTwa,vector<EdgewMPSBearing*>& ep,long infoWeight)
{
    if(pKFi->mwMPSBearing.empty())
        return;

    for(int j = 0; j < pKFi->mwMPSBearing.size(); j ++)
    {
        int channel = pKFi->mwMPSChannel[j];
        Eigen::Vector2d BearingAngle = pKFi->mwMPSAngle[j];
        //if(channel == 1 || channel == 3|| channel == 4|| channel == 5|| channel == 6)
        //    continue;

        int Trm = pKFi->mwMPSTransmitter[j];
        int i = 0;
        identifyTransmitter(Trm,i);
        //if(i==0||i ==2)
        //    continue;

        int td = abs(pKFi->mwMPStime[j] - pKFi->mTimeStamp*1e9);
        EdgewMPSBearing *e = new EdgewMPSBearing();
        e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
        g2o::SE3Quat gTwa = Converter::toSE3Quat(vTwa[i]);
        e->BearingAngle = BearingAngle;
        e->Twa = gTwa;
        e->channel = channel;
        e->mwMPSReceiver = pKFi->mwMPSReceiver;
        const g2o::VertexSE3Expmap * v1 = static_cast<const g2o::VertexSE3Expmap*>(optimizer.vertex(0));    //Tcw
        Eigen::Matrix<double, 2, 2> inforMatrix = Eigen::Matrix<double, 2, 2>::Identity();
        inforMatrix = infoWeight * inforMatrix;
...
template<class T>
void Optimizer::addwMPSBearingEdge(g2o::SparseOptimizer& optimizer,T lLocalKeyFrames,vector<cv::Mat>& vTwa,vector<EdgewMPSBearing*>& ep,long infoWeight)
{
    for(auto lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        //cout<< "mTimeStamp = " << pKFi->mTimeStamp <<endl;
        //cout<< "pKFi->mwMPSBearing.size() = " << pKFi->mwMPSBearing.size() <<endl;
        ...
        for(int j = 0; j < pKFi->mwMPSBearing.size(); j ++)
        {
            ...
            //if(channel == 1 || channel == 2|| channel == 3|| channel == 4|| channel == 5|| channel == 6)
            //    continue;
            int Trm = pKFi->mwMPSTransmitter[j];
            int i = 0;
            identifyTransmitter(Trm,i);
...
void Optimizer::identifyTransmitter(int Trm,int &i)
{
    switch(Trm)
    {
        case 1800:
...
void Optimizer::LocalwMPSDirBA(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, vector<cv::Mat>& vTwa, bool bScaleFixFlag, bool bMonocular, list<KeyFrame*> lKF, long weight)
{
    cout<< "Start Local wMPS Bearing BA" <<endl;
    ...
    //for(auto lit=lKF.begin(),lend=lKF.end();lit!=lend; lit++)
    //{
    //    KeyFrame* pKFi = *lit;
    ...
    int num_trans = vTwa.size();
    //cout << " num_trans = " <<num_trans <<endl;
    //wMPS Position scale estimation

    vector<EdgewMPSBearing*> ep;
    //ep.reserve(lLocalKeyFrames.size()+lFixedCameras.size()+lKF.size());

    addwMPSBearingEdge(optimizer, lLocalKeyFrames,vTwa,ep,weight);
    //addwMPSBearingEdge(optimizer, lFixedCameras,vTwa,ep,weight);
    //addwMPSBearingEdge(optimizer, lKF,vTwa,ep,weight);
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        int id = pMP->mnId+maxKFid+2+num_trans;   //reserve some space for transmitter vertex
        vPoint->setId(id);
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(pKFi->mpCamera2){
                    ...
                    if(rightIndex != -1 ){
                        ...
                        e->mTrl = Converter::toSE3Quat(pKFi->GetRelativePoseTrl());
    ...
    optimizer.optimize(10);   //this term is 5
    ...
    if(bDoMore)
    {
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend;i++)
        {
            ...
            if(e->chi2()>14.067 || !e->isDepthPositive())
            {
                e->setLevel(1);
                nStereoBadObs++;
            }
        ...
        //Remove these data from wMPS measurements
        for(size_t i=0, iend=ep.size(); i<iend;i++)
        {
            EdgewMPSBearing* e = ep[i];
            if(e->chi2()>50000)
            {
                e->setLevel(1);
            }
        ...
        // Optimize again
        optimizer.initializeOptimization(0);
        optimizer.optimize(20);   //the term is 10
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        //cout<< "After BA =" << t_inverse.transpose() <<endl;
        Sophus::SE3f SSE3f(SE3quat.rotation().cast<float>(),SE3quat.translation().cast<float>());
        pKFi->SetPose(SSE3f);
    }
    ...
    cout << "Vision-wMPS BA success!" << endl;

}

int Optimizer::wMPSBearingPoseOptimization(Frame *pFrame,vector<cv::Mat>& vTwa,long weight)
{
    //cout << " Start wMPS PnP " <<endl << endl;
    cout << "pFrame->mTimeStamp = " << pFrame->mTimeStamp << endl;
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
            ...
            if(pMP)
            {
                ...
                if(!pFrame->mpCamera2){
                    ...
                    if (i < pFrame->Nleft) {    //Left camera observation
                        ...
                        e->mTrl = Converter::toSE3Quat(pFrame->GetRelativePoseTrl());
    ...
    vector<EdgewMPSBearing*> ep;
    ep.reserve(100);
    addwMPSBearingPoseOptimizationEdge(optimizer,pFrame,vTwa,ep, weight);
    ...
    float chi2Stereo[4]={14.067,14.067,14.067, 14.067};
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        g2o::SE3Quat SSE3(Converter::toSE3Quat(pFrame->GetPose()));
        vSE3->setEstimate(SSE3);
        optimizer.initializeOptimization(0);
        ...
        for(size_t i=0, iend=ep.size(); i<iend; i++)
        {
            EdgewMPSBearing* e = ep[i];
            const float chi2 = e->chi2();
            //cout << "wMPS Bearing Observation chi2 = " << chi2 <<endl;
            if(chi2>50000)
                e->setLevel(1);
            else
            {
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            ...
            vchi2.push_back(e->chi2());
        }
        std::sort(vchi2.begin(), vchi2.begin() + vpEdgesStereo.size(), std::greater<double>());
        if(vpEdgesStereo.size() >10 && vchi2[10] > chi2Stereo[it])
        {
            chi2Stereo[it] = vchi2[10];
            cout << "chi2 threshold = "<< vchi2[10] <<endl;
        }
        */
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            ...
            const float chi2 = e->chi2();
            //cout << "vpEdgesStereo chi2 = " << chi2 <<endl;
    ...
    Sophus::SE3<float> SSE3(SE3quat_recov.rotation().cast<float>(),SE3quat_recov.translation().cast<float>());
    pFrame->SetPose(SSE3);

    //cout << " End wMPS PnP " <<endl << endl;

    return nInitialCorrespondences-nBad;
}

void Optimizer::GlobalViBundleAdjustement(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
                                         vector<cv::Mat>& vTwa, bool mbMonocular, int nIterations, long weight, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        mnKF.insert(pair<int,int>(vpKFs[i]->mnId,0));
    }

    int num_trans = vTwa.size();

    vector<EdgewMPSBearing*> ep;
    ep.reserve(vpKFs.size());

    if(!mbMonocular)
    {
        addwMPSBearingEdge(optimizer,vpKFs,vTwa,ep, weight);
    }
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->mpCamera2){
                ...
                if(rightIndex != -1 && rightIndex < pKF->mvKeysRight.size()){
                    ...
                    e->mTrl = Converter::toSE3Quat(pKF->GetRelativePoseTrl());
    ...
    //for(auto i = mnKF.begin();i != mnKF.end();i++)
    //{
    //    cout << " KF = " << i->first << " " << i->second <<endl;
    //}

    const float chi2Mono[4]={5.991,5.991,5.991,5.991};
    const float chi2Stereo[5]={7.815,7.815,7.815, 7.815,7.815};
    const int its[5]={20,20,20,20,20};

    optimizer.initializeOptimization();
    optimizer.optimize(20);

    // Optimize!
    //delete points whose chi2()>1.0
    int nBad=0;
    for(size_t it=0; it<5; it++)
    {
        ...
        for(size_t i=0, iend=vpEdgesStereo.size(); i<iend; i++)
        {
            ...
            if(chi2>chi2Stereo[it])
            {
                e->setLevel(1);
                nBad++;
                //cout << "e->vertex(1)->id() = " <<  e->vertex(1)->id() <<endl;
                //cout << "chi2 = " <<chi2 <<endl;
            }
            else
            {
        ...
        for(size_t i=0, iend=ep.size(); i<iend;i++)
        {
            ...
            {
                e->setLevel(1);
                //cout << "Larger e->chi2() = " << e->chi2() <<endl;
            }
        }
        if(optimizer.edges().size()<10)
            break;
    ...
    for (auto lit = vpKFs.begin(), lend = vpKFs.end(); lit != lend; lit++) {
        ...
        g2o::VertexSE3Expmap *vSE3 = static_cast<g2o::VertexSE3Expmap *>(optimizer.vertex(pKFi->mnId));
        g2o::SE3Quat SE3quat = vSE3->estimate();
        Sophus::SE3<float> SSE3(SE3quat.rotation().cast<float>(),SE3quat.translation().cast<float>());
        pKFi->SetPose(SSE3);
    }
...
bool Optimizer::GlobalVisualwMPSBundleAdjustment(Map* pMap, vector<cv::Mat>& vTwa, bool mbMonocular, int nIterations, long weight, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
    int Num_Dir = 0;
    for(auto i:vpKFs)
    {
        Num_Dir += i->mwMPSBearing.size();
    }
    //cout << "i->mwMPSBearing.size() = " << Num_Dir <<endl;

    if(Num_Dir<100)
    {
        cout << "Number of Bearing not enough" <<endl;
        return false;
    }
    //for(auto i:vpKFs)
    //{
    //    for(auto j : i->mwMPSBearing)
    //    {
    //        cout << "mTimeStamp = "<< i->mTimeStamp <<endl;
    //        cout << "mwMPSBearing = "<< j <<endl;
    //    }
    //}
    GlobalViBundleAdjustement(vpKFs,vpMP,vTwa,mbMonocular,nIterations, weight,pbStopFlag, nLoopKF, bRobust);
    return true;
}
