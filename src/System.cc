...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    int time = timestamp *1e9;
    std::string Left  = "./Remap/Left/L"  + std::to_string(time) + ".png";
    std::string Right = "./Remap/Right/R" + std::to_string(time) + ".png";

    if(!imLeftToFeed.empty() && !imRightToFeed.empty())
    {
        cv::imwrite(Left,imLeftToFeed);
        cv::imwrite(Right,imRightToFeed);
    }
...
void System::SaveTrackTrajectory(string SaveFileName)
{
    mpTracker->SaveTrackTrajectory(SaveFileName);
    return;
}


void System::LoadwMPSBearing(vector<double> wMPSvTimestamps, wMPS::Bearing* wMPSBearing)
{
    mpTracker->LoadwMPSBearing(wMPSvTimestamps,wMPSBearing);
    return;
}
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor==IMU_RGBD)
        {
            ...
            f << 1e9*pKF->mTimeStamp  << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        ...
        else
        {
            ...
            Eigen::Quaternionf q = Twc.unit_quaternion();
            Eigen::Vector3f t = Twc.translation();
            f << 1e9*pKF->mTimeStamp << " " <<  setprecision(9) << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
    }
    f.close();
}
