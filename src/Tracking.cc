    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);
    cv::Mat mTpa,mTpa2,mTpa3;
    cv::FileNode node = fSettings["Tpa"];
    vTpa.reserve(3);

    if(!node.empty())
    {
        mTpa = node.mat();
        if(mTpa.rows != 4 || mTpa.cols != 4)
        {
            std::cerr << "*Tpa matrix have to be a 4x4 transformation matrix*" << std::endl;
        }
        else
        {
            vTpa.push_back(mTpa);
    ...
    {
        std::cerr << "*Tpa matrix doesn't exist*" << std::endl;
    }

    cv::FileNode node2 = fSettings["Tpa2"];
    if(!node2.empty())
    {
        mTpa2 = node2.mat();
        if(mTpa2.rows != 4 || mTpa2.cols != 4)
        {
            std::cerr << "*Tpa2 matrix have to be a 4x4 transformation matrix*" << std::endl;
        }
        else
        {
            vTpa.push_back(mTpa2);
        }
    }
    else
    {
        std::cerr << "*Tpa2 matrix doesn't exist*" << std::endl;
    }

    cv::FileNode node3 = fSettings["Tpa3"];
    if(!node3.empty())
    {
        mTpa3 = node3.mat();
        if(mTpa3.rows != 4 || mTpa3.cols != 4)
        {
            std::cerr << "*Tpa3 matrix have to be a 4x4 transformation matrix*" << std::endl;
        }
        else
        {
            vTpa.push_back(mTpa3);
        }
    }
    else
    {
        std::cerr << "*Tpa3 matrix doesn't exist*" << std::endl;
    }

    cv::FileNode node_ReceiverPath = fSettings["wMPS.ReceiverPath"];
    if(!node_ReceiverPath.empty())
    {
        if(!node_ReceiverPath.empty() && node_ReceiverPath.isString())
        {
            string ReceiverPath;
            node_ReceiverPath>> ReceiverPath;
            GetwMPSReceivesInCameraFrame(ReceiverPath);
        }
    }
    else
    {
        std::cerr << "*wMPS ReceiverPath doesn't exist*" << std::endl;
    }
    mPnpWeight = 1e8;
...
void Tracking::Track()
{
    if (!mbwMPSInitialized)
    {
        mpLocalMapper->GetwMPStoWorld(vTpa);
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                ...
                if(bOK)
                {
                    if(mpLocalMapper->mbVisualwMPSBA)
                    {
                        Sophus::SE3f Twc = mCurrentFrame.GetPose().inverse();
                        Eigen::Quaternionf q = Twc.unit_quaternion();
                        Eigen::Vector3f twb = Twc.translation();
                        f << 1e9*mCurrentFrame.mTimeStamp << " " <<  setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
                    }
                }
...
void Tracking::GetAnchortoWorld(vector<cv::Mat> mvTwa)
{
    vTwa = mvTwa;
    return;
}
...
bool Tracking::TrackLocalMap()
{
    ...
    if (!mpAtlas->isImuInitialized())
    {
        //Optimizer::wMPSBearingPoseOptimization(&mCurrentFrame,vTwa,mPnpWeight);
        //Optimizer::PoseOptimization(&mCurrentFrame);
        if(!mpLocalMapper->mbVisualwMPSBA || mwMPSReceiver.size() == 0)
            Optimizer::PoseOptimization(&mCurrentFrame);
        else
            Optimizer::wMPSBearingPoseOptimization(&mCurrentFrame,vTwa,mPnpWeight);
    }
...
void Tracking::SaveTrackTrajectory(string SaveFileName)
{
    mSaveFileName = SaveFileName;
    f.open(mSaveFileName,std::ios::app);
    return;
}


void Tracking::LoadwMPSBearing(vector<double> wMPSvTimestamps, wMPS::Bearing *wMPSBearing)
{
    mvwMPSTimestamps = wMPSvTimestamps;
    mwMPSBearing = wMPSBearing;
    return;
}

int Tracking::GetwMPSReceivesInCameraFrame(string ReceiverPath)
{
    cout << "Start read wMPSReceivesInCameraFrame File" << endl;
    const string strSettingsFile = ReceiverPath;
    ifstream fTimes;
    fTimes.open(strSettingsFile);
    if(!fTimes)
    {
        cout << "Load wMPS Receivers File Failed" << endl;
        return 1;
    }
    while(!fTimes.eof()) {
        string s;
        ...
        if (!s.empty())
        {
            ...
            mwMPSReceiver.insert(make_pair<int,Eigen::Vector3d>(index,Eigen::Vector3d(x,y,z)));
        }
    }
    cout << " -------------- " << endl;
    cout << "Get " << mwMPSReceiver.size() << " Receiver Position in Camera Frame" <<endl;

    for(auto i : mwMPSReceiver)
...
void Tracking::GetwMPSBearingMeasurement()
{
    double FT = 1e6;
    ...
    for(int m = 0 ; m < mvwMPSTimestamps.size() ; m ++ )
    {
        //double t_wMPS = mvwMPSTimestamps[m] /1e9;
        //double t_wMPS_next = mvwMPSTimestamps[m+1] /1e9;
        double t_wMPS = mvwMPSTimestamps[m] /1e3;
        double t_wMPS_next = mvwMPSTimestamps[m+1] /1e3;

        if( abs(t_frame - t_wMPS) <= 0.002 )
        {
            mwMPSBearing[m].Bearing = mwMPSBearing[m].Bearing / mwMPSBearing[m].Bearing.norm();
            mCurrentFrame.mwMPSChannel.push_back(mwMPSBearing[m].channel);
            mCurrentFrame.mwMPSTransmitter.push_back(mwMPSBearing[m].transmitter);
            mCurrentFrame.mwMPStime.push_back(mwMPSBearing[m].time);
            mCurrentFrame.mwMPSBearing.push_back(mwMPSBearing[m].Bearing);
            mCurrentFrame.mwMPSAngle.push_back(mwMPSBearing[m].Angle);
        }
        else if(t_frame > t_wMPS  && t_frame < t_wMPS_next && (t_wMPS_next-t_wMPS)<=0.500)
        {
            double t1 = t_frame - t_wMPS;
            double t2 = t_wMPS_next - t_frame;
            double w1 = t2 / ( t1 + t2 );
            double w2 = t1 / ( t1 + t2 );
            mwMPSBearing[m].Bearing = mwMPSBearing[m].Bearing / mwMPSBearing[m].Bearing.norm();
            mwMPSBearing[m+1].Bearing = mwMPSBearing[m+1].Bearing / mwMPSBearing[m+1].Bearing.norm();
            double yita = acos(mwMPSBearing[m].Bearing.transpose() * mwMPSBearing[m+1].Bearing);
            double cof1 = sin(w1*yita)/sin(yita);
            double cof2 = sin(w2*yita)/sin(yita);
            Eigen::Vector3d line = cof1 * mwMPSBearing[m].Bearing + cof2 * mwMPSBearing[m+1].Bearing;
            double pi = 3.14159265358979323;
            line = line / line.norm();
            double s_z1 = line.y() / sqrt(line.x() * line.x() + line.y() * line.y());
            double c_z1 = line.x() / sqrt(line.x() * line.x() + line.y() * line.y());
            double horizon = 0.0, vertical = 0.0;
            if(s_z1 >= 0)
                if(c_z1 >= 0)
                    horizon = asin(s_z1);
                else
                    horizon = pi - asin(s_z1);
            else
                if(c_z1 >= 0)
                    horizon = asin(s_z1) + pi * 2.0;
                else
                    horizon = pi - asin(s_z1);
            vertical = atan(line.z() / sqrt(line.x() * line.x() + line.y() * line.y()));

            mCurrentFrame.mwMPSChannel.push_back(mwMPSBearing[m].channel);
            mCurrentFrame.mwMPSTransmitter.push_back(mwMPSBearing[m].transmitter);
            mCurrentFrame.mwMPStime.push_back(mwMPSBearing[m].time);
            mCurrentFrame.mwMPSBearing.push_back(line);
            mCurrentFrame.mwMPSAngle.push_back(Eigen::Vector2d(horizon,vertical));
        }
    }
    mCurrentFrame.mwMPSReceiver = mwMPSReceiver;
